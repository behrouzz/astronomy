...
new_edges = []
for i in edges:
    if (new_df.loc[i[0]]['alt']>0) and (new_df.loc[i[1]]['alt']>0):
        new_edges.append(i)

new_edges_star1 = [i[0] for i in new_edges]
new_edges_star2 = [i[1] for i in new_edges]

xy1 = new_df[['az', 'alt']].loc[new_edges_star1].values
xy2 = new_df[['az', 'alt']].loc[new_edges_star2].values
#lines_xy = np.rollaxis(np.array([xy1, xy2]), 1)
